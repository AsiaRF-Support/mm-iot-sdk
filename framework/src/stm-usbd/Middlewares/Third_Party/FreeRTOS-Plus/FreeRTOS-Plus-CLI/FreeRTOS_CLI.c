...
BaseType_t FreeRTOS_CLIRegisterCommand( const CLI_Command_Definition_t * const pxCommandToRegister )
{
	...
	pxNewListItem = ( CLI_Definition_List_Item_t * ) mmosal_malloc_( sizeof( CLI_Definition_List_Item_t ) );
...
BaseType_t prvHelpCommand( char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString )
{
	...
	size_t totalLen = 0;
	pcWriteBuffer[0] = '\0';
	const char *helpStr = pxCommand->pxCommandLineDefinition->pcHelpString;
	size_t helpLen = strlen(helpStr);

	while (pxCommand != NULL && totalLen + helpLen < xWriteBufferLen - 1) {
		strcat(pcWriteBuffer, helpStr);
		totalLen += helpLen;
		...
		helpStr = pxCommand->pxCommandLineDefinition->pcHelpString;
	...
	if( pxCommand == NULL )
	{
		...
		//strcat(pcWriteBuffer, pcEndOfOutputMessage);
	}
	else
	{
		xReturn = pdTRUE;
