...
void *USBD_static_malloc(uint32_t size)
{
	...
	//void *mem = pvPortMalloc_(sizeof(USBD_CDC_HandleTypeDef));
	void *mem = mmosal_malloc_(sizeof(USBD_CDC_HandleTypeDef));

	if (mem == NULL) {
        //printf("Error: pvPortMalloc failed to allocate memory for USB CDC\n");
        printf("Error: mmosal_malloc_ failed to allocate memory for USB CDC\n");
        return NULL;
    }
